...
#' @param resolution  Target bin size (bp).  If the native bins are smaller they will be merged;
#'                    if larger they will be sliced.
#' @param genome_package  
#'   Name of a BSgenome package (e.g. \code{"BSgenome.Dmelanogaster.UCSC.dm6"}) from which
#'   to extract DNA for GC calculations.
#' @param acc_wig     (Optional) Path to a DNase‑I wig (bedGraph) file.  If provided together
#'                    with \code{chain_file}, accessibility scores will be lifted over,
#'                    averaged per bin, and returned.
#' @param chain_file  (Optional) LiftOver chain file for mapping \code{acc_wig} coordinates.
#' @param te_granges  (Optional) Either
#'   \itemize{
...
#' @importFrom rtracklayer import.chain import
...
get_data <- function(file_path, chr, start, end, resolution,
  ...
  if (!is.null(acc_wig) && !is.null(chain_file)) {
    dnase_df <- read.table(acc_wig, header=FALSE, skip=1,
  ...
  full$ACC <- if (is.null(acc_gr)) {
    ...
    mapply(function(s1,e1,s2,e2) {
      ...
      m1 <- mean(subsetByOverlaps(acc_gr, g1)$score, na.rm=TRUE)
      m2 <- mean(subsetByOverlaps(acc_gr, g2)$score, na.rm=TRUE)
